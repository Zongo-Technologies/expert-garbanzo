...
export class DashboardService {
  ...
  async getStats(): Promise<QueueStats> {
    ...
    const scheduledResult = await this.pool.query(
      'SELECT COUNT(*) as count FROM que_jobs WHERE run_at > $1',
      [now]
    );
    const scheduled = parseInt(scheduledResult.rows[0].count);

    // Get ready jobs (past run_at)
    const readyResult = await this.pool.query(
      'SELECT COUNT(*) as count FROM que_jobs WHERE run_at <= $1',
      [now]
    );
    ...
    const failedResult = await this.pool.query(
      'SELECT COUNT(*) as count FROM que_jobs WHERE error_count > 0'
    );
    ...
    const avgErrorResult = await this.pool.query(
      'SELECT AVG(error_count) as avg FROM que_jobs WHERE error_count > 0'
    );
    ...
    const oldestResult = await this.pool.query(
      'SELECT run_at FROM que_jobs ORDER BY run_at ASC LIMIT 1'
    );
    const oldestJob = oldestResult.rows[0]?.run_at || null;

    const newestResult = await this.pool.query(
      'SELECT run_at FROM que_jobs ORDER BY run_at DESC LIMIT 1'
    );
  ...
  async deleteJob(jobId: number): Promise<boolean> {
    const result = await this.pool.query(
      'DELETE FROM que_jobs WHERE job_id = $1',
      [jobId]
    );
